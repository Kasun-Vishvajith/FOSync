...
import { format, addDays, nextMonday, nextFriday, isToday } from 'date-fns';
...
export default function AddEventModal({ isOpen, onClose, preselectedDate = null, preselectedType = null }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setTimeout(() => {
        setSuccess(false);
        setForm({ course_id: '', type: 'lecture', time: '10:00 AM', date: format(new Date(), 'yyyy-MM-dd'), customTitle: '' });
        onClose();
      }, 1200);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      {/* Modal Panel */}
      <div className="relative w-full sm:max-w-lg bg-[var(--color-surface)] rounded-t-[var(--radius-xl)] sm:rounded-[var(--radius-xl)] shadow-[var(--shadow-elevated)] border border-[var(--color-border)] animate-slide-up overflow-hidden">
              ...
              {dateSuggestions.map(({ label, date }) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${active ? 'bg-[var(--color-accent)] text-white border-transparent shadow-sm' : 'bg-[var(--color-bg-base)] text-[var(--color-text-secondary)] border-[var(--color-border)] hover:border-[var(--color-accent)]/40'}`}
                  >
                    {label}
                  </button>
                );
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {TIME_PRESETS.map(({ label, value }) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${form.time === value ? 'bg-[var(--color-accent)] text-white border-transparent shadow-sm' : 'bg-[var(--color-bg-base)] text-[var(--color-text-secondary)] border-[var(--color-border)] hover:border-[var(--color-accent)]/40'}`}
                >
                  {label}
                </button>
              ))}
